...
function! g:TimeAndBattery()
    ...
    let l:s .= system("bat=$((which acpi 1>/dev/null 2>/dev/null && acpi || (which pmset 1>/dev/null 2>/dev/null && pmset -g batt))|head -1|grep -o '[0-9]*%' |sed 's/%//');if [ -n \"$bat\" ]; then if [ $bat -lt 10 ]; then echo -n '  '; elif [ $bat -lt 25 ];then echo -n '  '; elif [ $bat -lt 50 ]; then echo -n '  '; elif [ $bat -lt 75 ]; then echo -n '  '; else echo -n '  '; fi; echo $bat%; fi")
    let l:s .= '%% '
    return l:s
endfunction
